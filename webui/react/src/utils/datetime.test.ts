    ...
    [
      {
        ...
        output: dayjs.utc(DATE[2]).add(dayjs(DATE[2]).utcOffset(), 'minute').format(FORMAT[0]),
      },
      {
        input: { date: DATE[3], options: { format: FORMAT[1], outputUTC: false } },
        output: dayjs.utc(DATE[3]).add(dayjs(DATE[3]).utcOffset(), 'minute').format(FORMAT[1]),
      },
    ].forEach(test => {
      const { date, options } = test.input;
    ...
    [
      ...
      },
      {
        ...
        output: dayjs
          ...
          .add(dayjs(DATE[3]).utcOffset(), 'minute')
          .format(utils.DEFAULT_DATETIME_FORMAT),
      },
    ].forEach(test => {
      const { date, options } = test.input;
