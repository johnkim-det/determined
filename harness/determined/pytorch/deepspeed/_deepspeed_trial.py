...
class DeepSpeedTrialController(det.TrialController):
    def __init__(self, trial_inst: det.Trial, *args: Any, **kwargs: Any) -> None:
        ...
        send_analytics("DeepSpeed Created")

        check.is_instance(trial_inst, DeepSpeedTrial, "DeepSpeedTrialController needs an DeepSpeedTrial")
        self.trial = cast(DeepSpeedTrial, trial_inst)
        self.context = cast(pytorch.deepspeed.DeepSpeedTrialContext, self.context)
    ...
    def _set_data_loaders(self) -> None:
        ...
        # Data parallel degree should be the same for all models in a trial.
        ...
        if self.context.mpu.should_build_data_loader():
            ...
            if isinstance(validation_data, pytorch.DataLoader):
                # We will repeat the validation loader so that we won't run into stop iteration
                # errors when used with pipeline parallelism.
                self.validation_loader = validation_data.get_data_loader(
                    repeat=True, skip=0, num_replicas=nreplicas, rank=rank
                )
            else:
                # Non-determined DataLoader; ensure the user meant to do this.
                if not self.context.experimental._data_repro_checks_disabled:
        ...
        # We will do a gather on training_loader length and broadcast to all slots.
        self.context._epoch_len = len(self.training_loader) if self.training_loader is not None else None
        ...
        if self.is_chief:
            ...
            if min(all_epoch_lens) < max(all_epoch_lens):
                logging.warning("Training dataloader length inconsistent across ranks. Using the minimum for epoch length.")
            self.context._epoch_len = min(all_epoch_lens) // self.context.get_num_micro_batches_per_batch()
        self.context._epoch_len = self.context.distributed._zmq_broadcast(self.context._epoch_len)

        self.num_validation_batches = len(self.validation_loader) if self.validation_loader is not None else None
        all_num_validation_batches = self.context.distributed._zmq_gather(self.num_validation_batches)
        if self.is_chief:
            ...
            if min(all_num_validation_batches) < max(all_num_validation_batches):
                logging.warning("Validation dataloader length inconsistent across ranks. Using the minimum for validation length.")
            self.num_validation_batches = min(all_num_validation_batches) // self.context.get_num_micro_batches_per_batch()
        self.num_validation_batches = self.context.distributed._zmq_broadcast(self.num_validation_batches)
    ...
    def _train_for_step(
        ...
        for batch_idx in range(start, end):
            ...
            batch_inputs = self.context.train_micro_batch_size_per_gpu * self.context.get_num_micro_batches_per_batch()
            num_inputs += batch_inputs
            num_train_step_calls = 1 if self.context.uses_pipeline_parallelism else self.context.get_num_micro_batches_per_batch()
            self.context._loss_ids = {}
            for micro_batch_idx in range(num_train_step_calls):
...
class DeepSpeedTrial(det.Trial):
    ...
    * **Clip gradients**.
    ...
    trial_context_class = pytorch.deepspeed.DeepSpeedTrialContext

    @abstractmethod
    def __init__(self, context: pytorch.deepspeed.DeepSpeedTrialContext) -> None:
        """
    ...
    @abstractmethod
    def train_batch(
        self, dataloader_iter: Iterator[torch.utils.data.DataLoader], epoch_idx: int, batch_idx: int
    ...
    def evaluate_batch(self, dataloader_iter: Iterator[torch.utils.data.DataLoader], batch_idx: int) -> Dict[str, Any]:
        """
    ...
    def save(
        self,
        context: pytorch.deepspeed.DeepSpeedTrialContext,
        path: pathlib.Path
    ):
